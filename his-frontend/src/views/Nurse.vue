        ...
        <el-tab-pane label="床位看板" name="beds">
          <div v-loading="bedLoading" class="bed-grid">
            ...
            >
              ...
              <div style="color:#667085;font-size:12px">{{ bed.ward || '' }}</div>
              <el-tag
                :type="bed.status === '空闲' ? 'success' : bed.status === '占用' ? 'primary' : 'danger'"
                style="margin-top:8px"
              >
                {{ bed.status }}
              </el-tag>
              <div v-if="bed.status === '占用'" style="margin-top:8px;font-size:13px">
            ...
            <el-table-column label="操作" width="190" fixed="right">
              <template #default="{ row }">
                <div class="order-actions">
                  ...
                  <!-- 可取消：待发药 / 执行中 / 已发药 -->
                  <el-popconfirm
                    v-if="['待发药', '执行中', '已发药'].includes(row.status)"
                    title="确认取消该医嘱？"
...
/** 医嘱状态（与后端 OrderStatus.value 保持一致） */
type OrderStatus = '待发药' | '执行中' | '已发药' | '已完成' | '已取消'
...
// ── 通用状态辅助函数（值与后端 OrderStatus.value 一致） ──
const ORDER_STATUS_MAP: Record<OrderStatus, { label: string; type: string }> = {
  '待发药': { label: '待发药', type: 'info' },
  '执行中': { label: '执行中', type: 'warning' },
  '已发药': { label: '已发药', type: 'primary' },
...
const loadInpatients = async () => {
  ...
  try {
    ...
    const res = await request.get('/api/nurse/inpatients', { params })
...
const handleDeptChange = () => {
  if (activeTab.value === 'beds') {
    ...
    loadInpatients()
  } else if (activeTab.value === 'orders' && selectedOrderPatientId.value) {
    orderPage.value = 1
    loadOrders()
  } else if (activeTab.value === 'vitals' && selectedVitalPatientId.value) {
    vitalPage.value = 1
    loadVitals()
  }
}
