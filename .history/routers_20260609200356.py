...
# ========== 全局日志工具函数（移出函数内部，避免重复创建） ==========
def _log_info(msg: str) -> None:
    print(f"[ASR INFO]  {msg}", file=sys.stderr, flush=True)
    logger.info(msg)


def _log_error(msg: str) -> None:
    print(f"[ASR ERROR] {msg}", file=sys.stderr, flush=True)
    logger.error(msg)


def _log_exception(msg: str) -> None:
    print(f"[ASR EXC]  {msg}", file=sys.stderr, flush=True)
...
# ========== 语音识别 (ASR) 【已全部修复】 ==========
# 前端 MediaRecorder 产生 WebM 容器（Opus 编码 48000Hz）
# 使用 Dashscope paraformer-v2 文件识别模型 + call(file=...) 同步模式
# Recognition.call() 是同步阻塞调用，通过 asyncio.to_thread 放入线程池，不阻塞事件循环
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """调用阿里云百炼 Paraformer 文件识别，返回 {"text", "duration", "error"}。

    流程：
        WebM 原始字节 → 临时文件 → Recognition.call(file=path) → 提取 text
    """
    ...
    if not audio_data:
        _log_error("接收到空音频数据")
        return {"text": "", "duration": 0.0, "error": "音频数据为空"}

    try:
        # 1. 写入临时文件（Windows 兼容，规避文件锁）
        ...
        _log_info(
            f"收到 {len(audio_data)} bytes, 文件={filename}, "
            f"临时文件大小={file_size}, tmp_path={tmp_path}"
        )

        # 2. 构建识别实例：恢复合法回调 + 匹配前端 48000 采样率
        recognition = Recognition(
            model="paraformer-v2",
            format="webm",
            sample_rate=48000,  # 修复：与前端 MediaRecorder 48000Hz 对齐
            callback=_NoOpRecognitionCallback(),  # 修复：禁止传 None，使用合法回调
        )
        _log_info(f"开始调用 Dashscope ASR (model=paraformer-v2, format=webm, sr=48000)...")

        # 同步阻塞方法放入线程池，不阻塞 FastAPI 事件循环
        result = await asyncio.to_thread(recognition.call, file=tmp_path)
        elapsed = time.perf_counter() - t0

        # 打印完整响应（调试用，上线可注释）
        _log_info(
            f"Dashscope 响应 — status={result.status_code}, "
        ...
        if result.status_code == 200:
            ...
            if text:
                _log_info(f"识别成功 — text='{text}' ({len(text)} 字)")
            else:
                _log_info("识别完成但 text 为空（可能音频无有效语音）")

            return {"text": text, "duration": round(elapsed, 2), "error": None}

        else:
            _log_error(
                f"识别失败 — status={result.status_code}, "
                f"code={getattr(result, 'code', '')}, "
                f"message={getattr(result, 'message', '')}"
            )
            return {"text": "", "duration": 0.0, "error": "语音识别服务暂时不可用，请稍后重试"}

    except Exception:
        _log_exception(f"调用异常")
        return {"text": "", "duration": 0.0, "error": "语音识别服务暂时不可用，请稍后重试"}
    finally:
        # 安全删除临时文件（增加存在判断）
        if tmp_path and os.path.exists(tmp_path):
            try:
                os.unlink(tmp_path)
                _log_info(f"临时文件已清理: {tmp_path}")
            except OSError as e:
                _log_error(f"临时文件清理失败: {str(e)}")
