...
# 流程：写入临时 .webm → ffmpeg 转码为 16000Hz 单声道 wav → 百炼 Paraformer → 返回文本


async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """使用阿里云百炼 Paraformer 实现语音转文字（北京区域），修复Windows兼容性"""
    ...
    try:
        ...
        wav_path = os.path.join(tempfile.gettempdir(), f"his_asr_{uid}.wav")
        with open(webm_path, "wb") as f:
            f.write(audio_data)
        _log(
            f"收到 {len(audio_data)} bytes | 文件={filename} | 大小={os.path.getsize(webm_path)} bytes"
        )

        # 3. ffmpeg 转码: webm → wav (16000Hz, mono, 16bit PCM) 【修复Windows兼容】
        _log("ffmpeg 转码中...")
        # 替换为线程池执行，解决Windows不支持异步子进程的问题
        ...
        _log(f"转码完成 | wav 大小={os.path.getsize(wav_path)} bytes")

        # 4. 调用百炼 Paraformer ASR（同步调用放线程池，不阻塞事件循环）
        _log(
            f"调用 Paraformer ASR (model={settings.DASHSCOPE_ASYNC_MODEL}, region={dashscope.region})"
        )
        recognition = Recognition(
            model=settings.DASHSCOPE_ASYNC_MODEL,
            format="wav",
            sample_rate=16000,
        )
        result = await asyncio.to_thread(recognition.call, file=wav_path)
        ...
        if result.status_code == 200:
            ...
            if result.output and result.output.get("sentence"):
                text = result.output["sentence"].get("text", "").strip()
            if text:
            ...
            _log_err(f"ASR 调用失败 | 状态码: {result.status_code} | 错误: {result.message}")
            return {"text": "", "duration": 0, "error": "语音识别服务暂时不可用，请稍后重试"}

    except subprocess.CalledProcessError:
        err_msg = "ffmpeg 转码失败，请检查录音文件或ffmpeg安装"
        _log_err(err_msg)
        return {"text": "", "duration": 0, "error": err_msg}
