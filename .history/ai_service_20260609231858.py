...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """用 qwen-audio-chat-v1 模型实现语音转文字（100%兼容版）"""
    ...
    try:
        ...
        # 3. 用 qwen-audio-chat-v1 模型处理音频（核心修改）
        log("调用 qwen-audio-chat-v1 模型处理音频...")
        messages = [
            {
                "role": "user",
                "content": [
                    {"audio": tmp_path},
                    {"text": "请把这段语音完整转写成文字，只返回识别结果，不要多余说明"},
                ],
            }
        ]
