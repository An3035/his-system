...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    ...
    try:
        # 修复Windows临时文件权限问题：先创建文件，写入后关闭，再传给SDK
        # tempfile.NamedTemporaryFile在Windows上会以独占模式打开，导致SDK无法读取
        tmp_file_path = os.path.join(tempfile.gettempdir(), f"asr_{uuid.uuid4().hex}.webm")
        with open(tmp_file_path, "wb") as f:
            f.write(audio_data)

        logger.info(f"ASR: 收到音频数据 {len(audio_data)} bytes, filename={filename}")
        logger.info(f"ASR: 临时文件已保存到 {tmp_file_path}")

        # 调用阿里云识别
        #    format="webm" 是关键：前端 MediaRecorder 输出 WebM 容器 + Opus 编码
        #    sample_rate=48000 与浏览器默认采样率一致
        ...
        # 同步调用，避免线程安全问题
        result = recognition.call(file=tmp_file_path)
        ...
        if result.status_code == 200:
            logger.info(f"ASR: 完整output结构: {result.output}")
            logger.info(f"ASR: get_sentence()返回: {result.get_sentence()}")
            ...
            try:
                ...
                logger.warning(f"ASR: get_sentence()解析失败: {e}")

            # 优先级2: 直接从output字典提取（兜底方案）
            if not text and hasattr(result, "output") and isinstance(result.output, dict):
                output = result.output
                if "sentences" in output and isinstance(output["sentences"], list):
                    text = "".join(
                        s.get("text", "") for s in output["sentences"] if isinstance(s, dict)
                    )
                elif "sentence" in output and isinstance(output["sentence"], dict):
                    text = output["sentence"].get("text", "")
                elif "text" in output and isinstance(output["text"], str):
                    text = output["text"]
                elif "transcript" in output and isinstance(output["transcript"], str):
            ...
            # 返回真实的错误信息给前端
            dashscope_code = getattr(result, "code", "UNKNOWN")
            dashscope_msg = getattr(result, "message", "未知错误")
            error_msg = f"Dashscope API错误: {dashscope_code} - {dashscope_msg}"
            logger.error(f"ASR: 识别失败 — {error_msg}")
            return {"text": "", "duration": 0, "error": error_msg}

    except Exception as e:
        logger.exception(f"ASR: 调用异常: {type(e).__name__}: {e}")
        return {"text": "", "duration": 0, "error": f"系统异常: {type(e).__name__}: {str(e)}"}
    finally:
        # 清理临时文件
        if tmp_file_path:
            try:
                os.unlink(tmp_file_path)
                logger.info(f"ASR: 临时文件已清理 {tmp_file_path}")
            except OSError as e:
                logger.warning(f"ASR: 临时文件清理失败 {tmp_file_path}: {e}")
