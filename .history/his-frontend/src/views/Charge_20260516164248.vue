...
const fetchPendingList = async () => {
  ...
  try {
    ...
    pendingList.value = presRes.data.items.map((item: any) => ({
      id: item.id,
      type: 'prescription' as const,
      no: item.pres_no,
      patient_name: '待显示', // 暂时显示这个，不影响功能
      total_amount: item.total_amount,
      pay_amount: item.total_amount,
      created_at: item.created_at
    }))

    pendingTotal.value = presRes.data.total
  } catch (err: any) {
...
const fetchHistoryList = async () => {
  ...
  try {
    ...
    historyList.value = presRes.data.items.map((item: any) => ({
      id: item.id,
      type: 'prescription' as const,
      no: item.pres_no,
    ...
    historyTotal.value = presRes.data.total
  } catch (err: any) {
