...
const send = async () => {
  ...
  try {
    ...
    formData.append('message', text || '请分析以下文件内容')
    formData.append('history', JSON.stringify(history))
    formData.append('context_type', 'general')
    // 如果有context_id也加上
    // formData.append('context_id', yourContextId)
    ...
    // 发送请求（注意不要加Content-Type头，浏览器会自动设置）
    const res = await request.post('/api/ai/chat', formData)
