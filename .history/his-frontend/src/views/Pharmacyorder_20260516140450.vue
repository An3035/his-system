...
const fetchPendingList = async () => {
  ...
  try {
    // 改成通用的/api/orders，通过status参数筛选待发药
    ...
    showError('获取待发药医嘱列表失败：' + (err.response?.data?.detail || err.message))
  } finally {
...
const fetchHistoryList = async () => {
  ...
  try {
    // 改成通用的/api/orders，通过status参数筛选已发药
    const res = await request.get('/api/orders', { 
      params: {
        ...historyQuery,
        status: '已发药'
    ...
    showError('获取发药历史列表失败：' + (err.response?.data?.detail || err.message))
  } finally {
...
const confirmDispense = async () => {
  ...
  try {
    // 改成后端实际存在的/execute接口
    await request.patch(`/api/orders/${dispenseOrder.value.order_id}/execute`)
    showSuccess('发药成功')
    ...
    showError('发药操作失败：' + (err.response?.data?.detail || err.message))
  } finally {
