...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """
    阿里云百炼语音识别（直接调用HTTP接口，彻底绕过SDK所有坑）
    华北2北京区域专用，原生支持WebM格式，不需要ffmpeg
    """
    ...
    try:
        ...
        # 2. 音频转Base64（直接在内存中处理，不写临时文件）
        audio_base64 = base64.b64encode(audio_data).decode("utf-8")
        _log("音频转Base64完成")

        # 3. 直接调用百炼HTTP接口（华北2北京固定地址）
        url = "https://dashscope.cn-beijing.aliyuncs.com/api/v1/services/audio-asr/fun-asr-2025-11-07/generation"
        headers = {
        ...
        if response.status_code == 200:
            ...
            _log_err(f"接口调用失败 | 状态码: {response.status_code} | 响应: {response.text}")
