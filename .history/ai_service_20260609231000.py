...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """调用阿里云百炼 Qwen3-ASR-Flash 语音识别模型，返回 {"text", "duration", "error"}。

    流程：WebM 原始字节 → 临时文件 → Recognition.call(file=path) → 提取 text

    Args:
        audio_data: 音频文件原始字节（WebM 容器，Opus 编码，16000Hz）
        filename: 原始文件名，仅用于日志

    Returns:
        {"text": str, "duration": float, "error": Optional[str]}
    """
    t0 = time.perf_counter()
    tmp_path = None

    # ═══════════════════════════════════════════════════════════
    # 日志：print() 到 stdout（uvicorn 终端 100% 可见）
    # ═══════════════════════════════════════════════════════════
    ...
    try:
        ...
        if not audio_data or len(audio_data) < 100:
            log_err(f"音频数据过小: {len(audio_data)} bytes（可能为静默或上传失败）")
            return {"text": "", "duration": 0, "error": "音频数据无效，请重新录音"}

        # ── 2. 写入临时文件（Windows 兼容模式） ────────────────
        ...
        # ── 3. 调用 Dashscope Recognition（核心修改：模型名） ──────────────────────
        #    Recognition.call() 是同步阻塞方法（内部走 WebSocket），
        #    必须用 asyncio.to_thread() 放到线程池，否则阻塞事件循环。
        recognition = Recognition(
            model="qwen3-asr-flash",  # ✅ 替换为 Qwen3-ASR-Flash 模型
            format="webm",
            sample_rate=16000,  # 与前端 MediaRecorder 输出的 16000Hz 采样率对齐
            callback=_NoOpRecognitionCallback(),  # 必须传入回调实例，禁止传 None
        )
        ...
        log(
            ...
            f"output_type={type(getattr(result, 'output', None)).__name__}"
        )
        ...
        if result.status_code == 200:
            ...
            if text:
                ...
                log("识别完成但 text 为空 — 音频中可能无有效语音或格式不匹配")

            return {"text": text, "duration": round(elapsed, 2), "error": None}

        else:
            # API 返回非 200 → 打印完整响应体供诊断
            log_err(
                f"Dashscope API 拒绝请求 | "
