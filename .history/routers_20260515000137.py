...
async def create_order(
    ...
    doctor_id = doctor.id if doctor else 1  # fallback
...
@order_router.get("", response_model=List[schemas.MedicalOrderOut])
async def list_orders(
    ...
    q = select(MedicalOrder)
    if admission_id:
        q = q.where(MedicalOrder.admission_id == admission_id)
    if order_type:
        q = q.where(MedicalOrder.order_type == order_type)
    result = await db.execute(q.order_by(MedicalOrder.created_at.desc()).limit(100))
    return result.scalars().all()
