...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """
    阿里云百炼语音识别（万能测试版）
    依次测试华北2北京区域所有可用的同步模型
    """
    t0 = time.perf_counter()
    webm_path = None

    # ✅ 按优先级测试，第一个能用的就会返回结果
    model_list = ["paraformer", "paraformer-16k", "fun-asr-realtime", "paraformer-realtime-v2"]
        ...
        for model_name in model_list:
            try:
                ...
                if result.status_code == 200:
                    text = ""
                    if result.output and result.output.get("sentences"):
                        text = "".join(
                            [s.get("text", "") for s in result.output["sentences"]]
                        ).strip()

                    if text:
                        elapsed = time.perf_counter() - t0
                        _log(f"✅ 模型 {model_name} 识别成功 → '{text}' | 耗时: {elapsed:.2f}s")
                        return {"text": text, "duration": round(elapsed, 2), "error": None}
                    else:
                        _log(f"模型 {model_name} 返回空结果，继续测试下一个")
                        continue
                else:
                    _log(f"模型 {model_name} 调用失败: {result.message}，继续测试下一个")
                    continue

            except Exception as e:
                _log(f"模型 {model_name} 异常: {str(e)}，继续测试下一个")
                continue

        # 所有模型都测试失败
        return {"text": "", "duration": 0, "error": "所有语音识别模型都不可用，请联系管理员"}
