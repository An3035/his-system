...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    ...
    try:
        ...
        if result.status_code == 200:
            ...
            # ✅ 关键：直接从result.output里拿，绕过get_sentence()
            ...
            return {"text": "", "duration": 0, "error": f"{result.code}: {result.message}"}

    except Exception as e:
        return {"text": "", "duration": 0, "error": f"异常：{str(e)}"}
    finally:
        if tmp_file_path:
            try:
