...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """
    阿里云百炼语音识别（同步转写，和控制台效果完全一致）
    使用paraformer-v1模型，华北2北京区域稳定可用
    """
    ...
    try:
        ...
        # 3. 调用同步转写接口（和百炼控制台完全一样）
        _log("调用百炼同步语音识别（paraformer-v1，和控制台效果一致）...")

        # 空回调即可满足SDK要求
        callback = RecognitionCallback()
        recognition = Recognition(
            model="paraformer-v1",  # ✅ 控制台用的就是这个模型，稳定可用
            callback=callback,
            format="webm",
            sample_rate=16000,
        )
        ...
        if result.status_code == 200:
            ...
            if result.output and result.output.get("sentences"):
                # 把所有句子拼接起来
                text = "".join([s.get("text", "") for s in result.output["sentences"]]).strip()
            ...
            _log_err(f"调用失败 | 状态码: {result.status_code} | 错误: {result.message}")
