...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """
    阿里云百炼新平台fun-asr语音识别（官方SDK正确用法）
    和控制台测试100%一致，SDK自动处理所有格式问题
    """
    ...
    try:
        ...
        _log("调用百炼新平台fun-asr语音识别SDK...")

        # ✅ 新平台官方正确的调用方式
        response = await asyncio.to_thread(
            AudioRecognition.call,
            model="fun-asr",
            audio_file=webm_path,
            format="webm",
            sample_rate=16000,
            language="zh-CN",
        )
        ...
        if response.status_code == 200:
            ...
            if response.output and response.output.get("sentence"):
                text = response.output["sentence"].get("text", "").strip()
            ...
            _log_err(f"调用失败 | 状态码: {response.status_code} | 错误: {response.message}")
            return {"text": "", "duration": 0, "error": f"语音识别服务错误: {response.message}"}
