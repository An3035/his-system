...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """
    阿里云百炼语音识别（最终版，和控制台100%一致）
    使用最新的同步API，上传即返回结果，不需要轮询
    """
    ...
    try:
        ...
        # 2. 直接调用最新的同步API（和控制台完全一样）
        url = "https://dashscope.aliyuncs.com/api/v1/services/audio-asr/fun-asr/generation"
        headers = {
            "Authorization": f"Bearer {settings.DASHSCOPE_API_KEY}",
            "Content-Type": "audio/webm",
            "X-DashScope-Parameter": '{"format":"webm","sample_rate":16000,"language":"zh-CN"}',
        }

        _log("调用百炼最新同步语音识别API...")
        # 直接把音频二进制数据放在请求体里发送
        response = await asyncio.to_thread(
            requests.post, url, headers=headers, data=audio_data, timeout=30
        )
        ...
        if response.status_code == 200:
            ...
            _log(f"API响应: {json.dumps(result, ensure_ascii=False)}")

            text = ""
            if result.get("output") and result["output"].get("sentence"):
                text = result["output"]["sentence"].get("text", "").strip()

            if text:
                _log(f"✅ 识别成功 → '{text}' ({len(text)}字) | 耗时: {elapsed:.2f}s")
            ...
            _log_err(f"调用失败 | 状态码: {response.status_code} | 响应: {response.text}")
            return {"text": "", "duration": 0, "error": f"语音识别服务错误: {response.text}"}
