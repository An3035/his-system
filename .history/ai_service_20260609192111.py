...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    ...
    try:
        ...
        if result.status_code == 200:
            ...
            # 🔥 关键修复：get_sentence()返回的是迭代器，必须先转成列表！
            sentences = list(result.get_sentence())

            logger.info(f"ASR: get_sentence()转列表后: {sentences}")

            if isinstance(sentences, list) and len(sentences) > 0:
                # 迭代器里的每个元素都是字典，包含text字段
                text = "".join(s.get("text", "") for s in sentences if isinstance(s, dict))

            logger.info(f"ASR: 识别成功 — 耗时 {elapsed:.2f}s, 文本: '{text}'")
