...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    ...
    try:
        ...
        print(f"✅ 临时文件: {tmp_file_path}")
        print(f"✅ 文件大小: {os.path.getsize(tmp_file_path)} 字节")
        ...
        print(f"📊 完整result对象: {result}")
        print(f"📊 result.output: {getattr(result, 'output', '不存在')}")
        print(f"📊 result.get_sentence(): {result.get_sentence()}")
        print("-" * 60 + "\n")
        ...
        if result.status_code == 200:
            ...
            if isinstance(output, dict):
                print("🔍 正在遍历output字典的所有键:")
                for key, value in output.items():
                    print(f"  - {key}: {value}")
                    if key.lower() in ["text", "transcript", "sentence", "content"] and isinstance(
                        value, str
                    ):
                        text = value
                    elif key == "sentences" and isinstance(value, list):
                        text = "".join(s.get("text", "") for s in value if isinstance(s, dict))
            ...
            error_msg = f"Dashscope错误: {getattr(result, 'code', '未知')} - {getattr(result, 'message', '未知错误')}"
