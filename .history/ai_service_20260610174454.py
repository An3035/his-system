...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    """
    阿里云百炼语音识别（终极可行版）
    用HTTP multipart/form-data直接上传文件，绕过所有SDK坑
    华北2北京区域专用，支持WebM格式
    """
    ...
    try:
        ...
        # 2. 直接用multipart/form-data上传文件（唯一正确的本地文件上传方式）
        ...
        data = {
            ...
            "parameters": json.dumps(
                {"format": "webm", "sample_rate": 16000, "language_hints": ["zh-CN"]}
            ),
        }
        ...
        if submit_response.status_code != 200:
            _log_err(f"提交任务失败: {submit_response.text}")
            return {"text": "", "duration": 0, "error": f"提交识别任务失败: {submit_response.text}"}

        submit_result = submit_response.json()
        task_id = submit_result["output"]["task_id"]
        _log(f"任务提交成功，任务ID: {task_id}")
        ...
        for i in range(max_retries):
            _log(f"查询任务状态 ({i+1}/{max_retries})...")
            result_response = await asyncio.to_thread(
                requests.get, result_url, headers=headers, timeout=30
            )
            ...
            if task_status == "SUCCEEDED":
                ...
                _log_err(f"任务失败: {error_msg}")
                return {"text": "", "duration": 0, "error": f"识别任务失败: {error_msg}"}
        ...
        # 超时
        _log_err("任务处理超时")
        return {"text": "", "duration": 0, "error": "识别任务超时，请稍后重试"}
