...
async def asr_transcribe(audio_data: bytes, filename: str = "audio.webm") -> dict:
    ...
    try:
        ...
        # 把音频转成Base64，用JSON格式发送（和控制台的请求格式一致）
        import base64

        audio_base64 = base64.b64encode(audio_data).decode("utf-8")
        payload = {
            "input": {"audio": audio_base64},
            "parameters": {"format": "webm", "sample_rate": 16000, "language": "zh-CN"},
        }

        _log("调用百炼同步语音识别API（JSON格式）...")
        ...
        _log(f"API响应状态码: {response.status_code}")
        _log(f"API响应内容: {response.text}")
        ...
        if response.status_code == 200:
            ...
            error_msg = response.json().get("message", "请求格式错误")
            _log_err(f"调用失败: {error_msg}")
            return {"text": "", "duration": 0, "error": f"语音识别服务错误: {error_msg}"}
