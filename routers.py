    ...
    return [
        {
            ...
            "type": "prescription",
            # 患者信息
            "patient_name": p.registration.patient.name if p.registration and p.registration.patient else None,
            "patient_no": p.registration.patient.patient_no if p.registration and p.registration.patient else None,
            "gender": p.registration.patient.gender if p.registration and p.registration.patient else None,
            # 药品明细
            "drug_items": [
                {
    ...
    return [
        {
            ...
            "drug": {
                ...
                "purchase_price": float(r.drug.purchase_price) if r.drug and r.drug.purchase_price else 0,
...
async def transfer_to_pharmacy(
    ...
    if not wh_inv or wh_inv.stock_qty < quantity:
        raise HTTPException(400, f"药库库存不足（当前: {float(wh_inv.stock_qty) if wh_inv else 0}，需要: {quantity}）")
    ...
    await db.commit()
    return {"message": f"调拨成功：{drug.name} × {quantity} 从 {warehouse_type} 调拨至 {pharmacy_type}"}
...
async def ai_chat(
    ...
    if is_new_session and not session.title:
        ...
        background_tasks.add_task(
            background_generate_title, session.id, body.message
        )
...
async def list_sessions(
    ...
    # 总数
    count_q = select(func.count()).select_from(AiSession).where(
        AiSession.user_id == current_user.id
    )
...
async def ai_triage(
    ...
    for d in departments:
        doctors = [
            ...
            for doc in (d.doctors or []) if doc.is_active
        ]
