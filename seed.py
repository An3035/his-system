...
async def seed():
    ...
    async with SessionLocal() as db:
        ...
        # ── 药房库存（匹配药品ID: 阿莫西林=1 ~ 六味地黄丸=8） ──
        pharmacy_inventory = [
            PharmacyInventory(drug_id=1, stock_qty=200, alert_qty=50, pharmacy_type="西药房"),
            PharmacyInventory(drug_id=2, stock_qty=150, alert_qty=30, pharmacy_type="西药房"),
            PharmacyInventory(drug_id=3, stock_qty=300, alert_qty=100, pharmacy_type="西药房"),
            PharmacyInventory(drug_id=4, stock_qty=300, alert_qty=100, pharmacy_type="西药房"),
            PharmacyInventory(drug_id=5, stock_qty=100, alert_qty=20, pharmacy_type="西药房"),
            PharmacyInventory(drug_id=6, stock_qty=500, alert_qty=100, pharmacy_type="中药房"),
            PharmacyInventory(drug_id=7, stock_qty=500, alert_qty=100, pharmacy_type="中药房"),
            PharmacyInventory(drug_id=8, stock_qty=80, alert_qty=20, pharmacy_type="中药房"),
        ]
        db.add_all(pharmacy_inventory)

        # ── 药库库存 ────────────────────────────────────────────
        warehouse_inventory = [
            WarehouseInventory(drug_id=1, stock_qty=1000, warehouse_type="西药库"),
            WarehouseInventory(drug_id=2, stock_qty=800, warehouse_type="西药库"),
            WarehouseInventory(drug_id=3, stock_qty=1500, warehouse_type="西药库"),
            WarehouseInventory(drug_id=4, stock_qty=1500, warehouse_type="西药库"),
            WarehouseInventory(drug_id=5, stock_qty=500, warehouse_type="西药库"),
            WarehouseInventory(drug_id=6, stock_qty=2000, warehouse_type="中药库"),
            WarehouseInventory(drug_id=7, stock_qty=2000, warehouse_type="中药库"),
            WarehouseInventory(drug_id=8, stock_qty=400, warehouse_type="中药库"),
        ]
        ...
        # ── 床位 ──────────────────────────────────────────────
        beds = []
        for ward_num in range(1, len(departments) + 1):
